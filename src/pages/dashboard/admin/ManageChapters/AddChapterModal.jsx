...
const AddChapterModal = ({ book, onClose, onSuccess, refetchBooks }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Create chapter with the PDF URL
      createChapter({
        bookId: book._id,
  ...
  // Complete the handleDrop function
  const handleDrop = (e) => {
    e.preventDefault();
    setIsDragging(false);
      ...
      <div className="bg-white w-full max-w-xl p-6 rounded-xl shadow-lg" onClick={(e) => e.stopPropagation()}>
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold text-gray-900">
            Add Chapter to <span className="text-primary">{book.name}</span>
          </h2>
              ...
              {!pdfFile ? (
                <div className="py-4">
                  ...
                  <p className={`text-sm ${isDragging ? "text-primary font-medium" : "text-gray-600"}`}>
                    {isDragging ? "Drop your PDF file here" : "Click or drag & drop to upload a PDF file"}
                  </p>
            ...
            >
              {isSubmitting || isUploading ? (
                <span className="flex items-center">
                  ...
                  {isUploading ? `Uploading ${uploadProgress.toFixed(0)}%` : "Adding..."}
                </span>
              ) : (
                "Add Chapter"
              )}
